...
/* ============================================================
   OV3RSTREAM FR — addon Stremio perso
   Page brandée + AllDebrid + sources publiques (YTS / EZTV)
   100% ton code, aucun "AIOStreams" / "StreamFusion".
   ============================================================ */
...
/* ---------- sources publiques ---------- */
// Films via YTS (API JSON publique)
async function ytsMovie(imdb) {
  try {
    const j = await fetchJSON(`https://yts.mx/api/v2/list_movies.json?query_term=${imdb}`);
    const m = j?.data?.movies?.[0];
...
// Séries via EZTV (API JSON publique)
async function eztvEpisode(imdb, season, episode) {
  try {
    const id = imdb.replace('tt', '');
    const j = await fetchJSON(`https://eztvx.to/api/get-torrents?imdb_id=${id}&limit=100`);
...
/* ---------- AllDebrid ---------- */
const AD = 'https://api.alldebrid.com/v4';
async function adResolve(apikey, hash, wantFileRe) {
  // 1) upload magnet
  const up = await fetchJSON(`${AD}/magnet/upload?agent=${AGENT}&apikey=${apikey}&magnets[]=${hash}`);
  const info = up?.data?.magnets?.[0];
  if (!info || info.error) return null;
  const id = info.id;
  if (info.ready === false && !info.instant) {
    // pas en cache -> on ne bloque pas le stream, on skip
    return null;
  }
  // 2) status -> fichiers
  const st = await fetchJSON(`${AD}/magnet/status?agent=${AGENT}&apikey=${apikey}&id=${id}`);
  const files = st?.data?.magnets?.links || st?.data?.magnets?.[0]?.links || [];
  if (!files.length) return null;
  ...
  const pick = (vids.length ? vids : files).sort((a, b) => (b.size || 0) - (a.size || 0))[0];
  if (!pick) return null;
  // 3) unlock -> lien streamable
  const un = await fetchJSON(`${AD}/link/unlock?agent=${AGENT}&apikey=${apikey}&link=${encodeURIComponent(pick.link)}`);
  const url = un?.data?.link;
  if (!url) return null;
  return { url, size: pick.size };
}

/* ---------- manifest ---------- */
function manifest(host) {
  return {
    id: ADDON_ID,
    version: '1.0.0',
    name: ADDON_NAME,
...
app.get('/:cfg/stream/:type/:id.json', async (req, res) => {
  ...
  if (!cfg || !cfg.ad) return res.json({ streams: [] });
  const { type } = req.params;
  const parts = req.params.id.split(':'); // tt123:1:2
  const imdb = parts[0];
  ...
  torrents = torrents.sort((a, b) => (b.seeds || 0) - (a.seeds || 0)).slice(0, 8);

  const streams = [];
  await Promise.all(torrents.map(async (t) => {
    try {
      const r = await adResolve(cfg.ad, t.hash, null);
      if (r) {
        streams.push({
          name: `OV3RSTREAM\n${t.quality}`,
          title: `${t.name}\n⚡ AllDebrid · 👤 ${t.seeds || '?'} · 💾 ${human(t.size)}`,
  ...
  // tri par qualité
  const order = { '2160p': 4, '1080p': 3, '720p': 2, 'SD': 1 };
  streams.sort((a, b) => (order[b.name.split('\n')[1]] || 0) - (order[a.name.split('\n')[1]] || 0));
  res.json({ streams });
});
...
*{box-sizing:border-box}body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Arial,sans-serif;
background:radial-gradient(1100px 620px at 80% -10%,#211d40 0%,var(--bg) 55%);color:var(--ink);min-height:100vh}
.wrap{max-width:560px;margin:0 auto;padding:54px 22px 80px;text-align:center}
img.logo{width:132px;height:132px;filter:drop-shadow(0 10px 30px rgba(124,92,255,.45))}
h1{font-size:30px;margin:16px 0 4px;font-weight:800;letter-spacing:.5px}
h1 .c{color:var(--acc2)}
.sub{color:var(--mut);font-size:15px;margin:0 auto;max-width:420px}
.card{background:linear-gradient(180deg,var(--panel),#12131d80);border:1px solid var(--line);border-radius:18px;padding:26px 24px;margin:28px 0;text-align:left}
label{font-size:13px;color:var(--mut);text-transform:uppercase;letter-spacing:.08em}
input{width:100%;margin-top:8px;padding:13px 14px;border-radius:11px;border:1px solid var(--line);background:#0e0f18;color:#fff;font-size:15px}
input:focus{outline:none;border-color:var(--acc)}
.hint{font-size:13px;color:var(--mut);margin-top:8px}
.hint a{color:var(--acc2)}
button{width:100%;margin-top:18px;padding:14px;border:0;border-radius:12px;background:linear-gradient(90deg,var(--acc),#5a3fe0);color:#fff;font-size:16px;font-weight:700;cursor:pointer}
...
.row{display:flex;gap:10px;margin-top:10px;flex-wrap:wrap}
.btn2{flex:1;text-align:center;padding:12px;border-radius:11px;text-decoration:none;font-weight:700;font-size:14px}
.install{background:var(--acc2);color:#04212a}
.copy{background:#22253a;color:#fff;border:1px solid var(--line);cursor:pointer}
...
<div class="wrap">
  <img class="logo" src="/public/logo.png" alt="OV3RSTREAM FR">
  <h1>OV3R<span class="c">STREAM</span> FR</h1>
  <p class="sub">Ton addon Stremio perso. Films & séries en streaming direct via AllDebrid. Colle ta clé, installe, profite.</p>
  <div class="badge">🎬 Streaming · AllDebrid</div>

  <div class="card">
    <label for="ad">Clé API AllDebrid</label>
    <input id="ad" type="text" placeholder="Colle ta clé AllDebrid ici" autocomplete="off" spellcheck="false">
    <div class="hint">Pas encore de clé ? Génère-la sur <a href="https://alldebrid.com/apikeys/" target="_blank">alldebrid.com/apikeys</a> — c'est ta clé perso, elle reste dans ton lien d'installation.</div>
    <button id="go">Générer mon lien d'installation</button>

    <div class="out" id="out">
      <label>Ton lien d'installation</label>
      <div class="linkbox" id="murl"></div>
      <div class="row">
        <a class="btn2 install" id="install" href="#">📲 Installer dans Stremio</a>
        <button class="btn2 copy" id="copy">📋 Copier le lien</button>
      </div>
    </div>
  </div>

  <div class="foot">OV3RSTREAM FR — addon perso · hébergé par toi · propulsé par AllDebrid</div>
</div>
<script>
  const host = location.origin;
  ...
  document.getElementById('copy').onclick = ()=>{
    navigator.clipboard.writeText(document.getElementById('murl').textContent);
    document.getElementById('copy').textContent='✓ Copié';
    setTimeout(()=>document.getElementById('copy').textContent='📋 Copier le lien',1500);
  };
</script>
</body></html>`;
}
